<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %>
<!DOCTYPE html>
<html>
<head>
    <title>Adicionar Item à Venda</title>
    <style>
        body { font-family: Arial, sans-serif; margin: 40px; background-color: #f4f4f4; }
        .container { background: white; padding: 20px; border-radius: 8px; box-shadow: 0 0 10px rgba(0,0,0,0.1); max-width: 500px; margin: auto; }
        label { font-weight: bold; display: block; margin-top: 15px; }
        select, input { width: 100%; padding: 10px; margin: 5px 0 10px 0; border: 1px solid #ddd; border-radius: 4px; box-sizing: border-box; }
        button { background-color: #e65100; color: white; padding: 12px; border: none; border-radius: 4px; cursor: pointer; width: 100%; font-size: 16px; font-weight: bold; }
        button:hover { background-color: #bf360c; }
    </style>
</head>
<body>
<div class="container">
    <h2>Vincular Produto à Venda</h2>
    
    <form action="itemvenda" method="post">
        
        <label>ID da Venda:</label>
        <select name="idVenda" required>
            <option value="">-- Selecione a Venda --</option>
            <c:forEach var="v" items="${listaVenda}">
                <option value="${v.idVenda}">Venda Nº ${v.idVenda} (Cliente: ${v.idCliente})</option>
            </c:forEach>
        </select>

        <label>Produto:</label>
        <select name="idProduto" required>
            <option value="">-- Selecione o Produto --</option>
            <c:forEach var="p" items="${listaProdutos}">
                <option value="${p.idProduto}">${p.nome}</option>
            </c:forEach>
        </select>

        <label>Quantidade:</label>
        <input type="number" name="quantidade" min="1" required>

        <label>Preço Unitário (R$):</label>
        <input type="text" name="preco" placeholder="Ex: 5.50" required>

        <button type="submit">Adicionar Item</button>
    </form>
    <br>
    <a href="itemvenda?acao=listar" style="text-decoration: none; color: #666;">Voltar para Detalhes</a>
</div>
</body>
</html>